...
class SecondActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnStart.setOnClickListener {
            ...
            if(count !in 5..1000){
                Toast.makeText(this,"Enter list size between 5-1000",Toast.LENGTH_LONG).show()
                return@setOnClickListener
            }
            sleepTime = when (count) {
    ...
    private fun setSpinners() {
        ...
        spDesign.onItemSelectedListener = object : AdapterView.OnItemSelectedListener{
            override fun onNothingSelected(parent: AdapterView<*>?) { linearLayout.gravity = Gravity.CENTER_HORIZONTAL and Gravity.BOTTOM }
            ...
            override fun onItemSelected(
                ...
                when(parent?.getItemAtPosition(position).toString()) {
                    "Bar Graph Design" -> { linearLayout.gravity = Gravity.CENTER_HORIZONTAL and Gravity.BOTTOM }
    ...
    private fun doMergeSorting() {
        GlobalScope.launch(Dispatchers.IO) {
            mergeSort(0, count - 1)
            for (i in listInt) Log.d("Debug", "$i")
        }
    }
    ...
    private suspend fun mergeSort(l: Int, r: Int) {
        if (l < r) {
            val m = l + (r - l) / 2
            Log.d("Debug: ", "mergeSort($l,$m)")
            mergeSort(l, m)
            Log.d("Debug: ", "mergeSort(${m + 1},$r)")
            mergeSort(m + 1, r)
            Log.d("Debug: ", "merge($l,$m,$r)")
            merge(l, m, r)
        }
    }
